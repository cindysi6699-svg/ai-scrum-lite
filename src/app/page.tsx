        ...
        <div className="grid flex-1 gap-8 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div className="max-w-2xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-800">
              ...
              App and database schema are ready
            </div>
          ...
          <aside className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-6 rounded-md border border-amber-200 bg-amber-50 p-4">
              {isSignedIn ? (
                <>
                  <p className="text-sm font-semibold text-amber-900">Signed in</p>
                  <p className="mt-3 text-sm leading-6 text-amber-950">
                    {session?.user?.name ?? session?.user?.email ?? "GitHub user"} is
                    authenticated. The next MVP step is creating the first project
                    workspace.
                  </p>
                </>
              ) : (
